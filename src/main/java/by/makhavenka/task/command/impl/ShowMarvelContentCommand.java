...
/**
 * Сommand  shows marvel content.
 * Throws CommandException if invalid
 */
public class ShowMarvelContentCommand implements Command {

    private static final Logger lOGGER=LogManager.getLogger(ShowMarvelContentCommand.class);

    /**
     * The command creates ContentService and transfers content to service
     * uses method -- showMarvelContent
     * @param content
     * @return router
    ...
    public Router execute(RequestController content) throws CommandException {
        ...
        Router router;
        try{
            result = service.showMarvelContent(content);
        }catch (ServiceException e){
            throw new CommandException("error 'showMarvelContentCommand'",e);
        }

        lOGGER.info("The 'ShowMarvelContentCommand' executed successfully");
